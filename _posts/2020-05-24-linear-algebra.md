...
**Span.** We are given two vectors $$\begin{bmatrix}0 & 1\end{bmatrix}^T$$ and $$\begin{bmatrix}1 & 0\end{bmatrix}^T$$. These two vectors lie on the subspace of the two-dimensional vector space where $$x>=0$$ and $$y>=0$$. If we pick any vector from this subspace, it will be a linear combination of these two vectors. That is why these two vectors are said to span the subspace. Vectors $$v_1, v_2, \dots v_n$$ are said to span a space when the space consists of all linear combinations of those vectors. If vectors $$v_1, v_2, \dots v_k$$ span a space $$S$$, then $$S$$ is the smallest space containing those vectors.  

**Basis.** If we are given a sequence of vectors $$v_1, v_2, \dots , v_n$$ such that they are independent and they span a vector space, then these vectors are said to form the basis for that vector space. Given a vector space, every basis of that space has the same number of vectors. That number is called the dimension of that space. There are exactly $$n$$ vectors in every basis for the n-dimensional space.
...
The null space of a matrix $$A$$ contains all the solution vectors $$\vec{x}$$ to the equation $$A\vec{x}=0$$. $$A\vec{x}$$ is the linear combination of the column vectors of $$A$$. The column vectors of $$A$$ are independent only when the null space of $$A$$ contains only the zero vector.
...
For the matrix $$A=\begin{bmatrix} 1 & 3\\ 2 & 3\\ 4 & 1\end{bmatrix}$$, the row space will contain all the linear combinations of the rows $$\begin{bmatrix} 1 & 3\end{bmatrix}$$ and $$\begin{bmatrix} 2 & 3\end{bmatrix}$$ and $$\begin{bmatrix} 4 & 1\end{bmatrix}$$  of the matrix.
...
- the basis for the row space is the pivot columns in the matrix $$A^T$$ as they are the independent columns.

- the number of vectors in the basis is the number of vectors in the pivot columns of the matrix which is called the rank of the matrix. Therefore the dimension of the row space is equal to the rank of the matrix. The rank of the matrix $$A$$ is equal to the rank of the matrix $$A^T$$. Therefor the row space and the column space of a matrix $$A$$ has the same dimension.

## Null Space of $$A^T$$

This is also called the left null space of $$A$$. The null space of the matrix $$A^T$$ contains all the solutions ($$y$$) to the equation $$A^Ty=0$$. $$A^Ty$$ is the linear combination of the row vectors of $$A$$ or the column vectors of $$A^T$$.
...
- We know that the rank of matrix $$A$$ is the same as the rank of matrix $$A^T$$. Therefore the number of free columns of matrix $$A^T$$ will be $$m-r$$. These free columns gives us the special solutions. These special solutions form the basis of the null space of $$A^T$$.
...
Now that we understand the four fundamental subspaces, we can define the first part of the fundamental theorem of Linear Algebra.

The first part of the fundamental theorem of Linear Algebra states that:

- The column space and row space both have dimension $$r$$.

- The nullspace of a matrix and the left nullspace of that matrix have dimensions $$n-r$$ and $$m-r$$.
...
If we have a vector $$\overrightarrow{b}$$ and a line determined by a vector $$\overrightarrow{a}$$, we do the projection of $$\overrightarrow{b}$$ to find the vector $$\overrightarrow{p}$$ on line $$\overrightarrow{a}$$ that is closest to $$\overrightarrow{b}$$.

Here, the vector $$\overrightarrow{p}$$ is a scalar multiple of vector $$\overrightarrow{a}$$.

$$\overrightarrow{p}=x\overrightarrow{a}$$

Now from the figure we can see that the vector $$\overrightarrow{a}$$ is orthogonal to the vector $$\overrightarrow{e}$$. Therefore

$$\overrightarrow{a^T}\overrightarrow{e}=0$$

$$\implies \overrightarrow{a^T}(\overrightarrow{b}-\overrightarrow{p})=0$$

$$\implies \overrightarrow{a^T}(\overrightarrow{b}-x\overrightarrow{a})=0$$

$$\implies x\overrightarrow{a^T}\overrightarrow{a}=\overrightarrow{a^T}\overrightarrow{b}$$

$$\implies
x=\dfrac{\overrightarrow{a^T}\overrightarrow{b}}{\overrightarrow{a^T}\overrightarrow{a}}$$

Therefore we can write

$$\overrightarrow{p}=\overrightarrow{a}x \implies \overrightarrow{p}=\dfrac{\overrightarrow{a}\overrightarrow{a^T}\overrightarrow{b}}{\overrightarrow{a^T}\overrightarrow{a}}$$

If we double the vector $$\overrightarrow{b}$$, $$\overrightarrow{p}$$ doubles. However, if we double $$\overrightarrow{a}$$, $$\overrightarrow{p}$$ remains unchanged.
...
<img class="special-img-class" style="width:80%" src="/assets/img/linear_regression.jpg" />
...
<img class="special-img-class" style="width:80%" src="/assets/img/IMG_0062.jpg" />
...
<img class="special-img-class" style="width:100%" src="/assets/img/orthogonal.jpg" />
...
b & c \end{bmatrix},$ then $x^TAx=ax_1^2+2bx_1x_2+cx_2^2$. The matrix $A$ being positive definite means that the function $f(x)$ has a unique minimum at $x=0$ and is strictly positive for all other $x's$.
...
To solve a system of linear equations we need to solve for the unknowns. Elimination is the technique most commonly used by computer softwares to solve systems of linear equations. It finds a solution $$\vec{x}$$ to $$A\vec{x}=b$$ whenever $$A$$ is [invertible](#inverse-matrix).
...
Given a matrix $$A$$, the rank of the matrix is equivalent to the number of pivots in the echelon form of the matrix. The rank of the matrix tells us the number of independent rows or columns in the matrix.
...
The **rank** of a matrix is equal to the number of pivots it has.
In our example, the rank of matrix $$A$$ is $$2$$ as it has $$2$$ pivots in its echelon form.

## Solving $$Ax=0$$

In our matrix $$A$$, columns $$1$$ and $$3$$ are the pivot columns while columns $$2$$ and $$4$$ are the free columns. Therefore in our solution vector
...
$$x=\begin{bmatrix}
-2 & 1 & 0 & 0\end{bmatrix}^T$$. The solution vector means that $$-2$$ times the first column added with the second column will give us the zero vector. Therefor any multiple of the solution vector $$x$$ will also give the zero vector and thus a solution.
